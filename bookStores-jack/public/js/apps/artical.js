require([
  ...
  'base64',
  'render'
], function($, locationStorage, locationFormat, jsonp, base64, render) {
  function Init() {
    //定义全局变量
    ...
    if (
      locationStorage.get('tagBg') &&
      locationStorage.get('tagBg') == '夜晚'
    ) {
      this.isLight = false
    }
    if (this.isLight) {
    ...
    this.articalCon.on('click', function() {
      ...
      $('dl.chapterList').on('click', function() {
        window.location.href =
          '../../page/chapterList.html?fiction_id=352876&chapterNum=' +
          that.initChapterNum
      })
    })
  }
  ...
  Init.prototype.setFontSize = function() {
    ...
    $('button.large-btn').on('click', function() {
      if (that.initFSize < 30) {
        ...
        $('section.artical-con p').css(
          'font-size',
          that.initFSize / 37.5 + 'rem'
        )
      }
    })
