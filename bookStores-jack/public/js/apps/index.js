require(["jquery", "swiper", "bscroll", "render", "text!bookTBtpl", "text!bookLRtpl", "text!bookLRLimitTpl", "locationStorage"], function ($, Swiper, bscroll, render, bookTBtpl, bookLRtpl, bookLRLimitTpl, locationStorage) {
	function init() {  
	...
	init.prototype.headTabClick=function () {  
		...
		$("header div.swiper-tab span.tab_item").on("click",function () {  
			...
			$(this).addClass("head_tab_color").siblings().removeClass("head_tab_color");
		...
		this.conSwipers = new Swiper("#content",{
			on:{
				slideChangeTransitionStart: function () {
					...
					$("header div.swiper-tab span.tab_item").eq(Ind).addClass("head_tab_color").siblings().removeClass("head_tab_color");
				}
			}
		});
	...
	init.prototype.conScroll=function () {  
		...
		this.conScrolls.on("scroll", function () {
			if (this.y < this.maxScrollY - realSize) { //上拉
	...
	init.prototype.cityBanner=function (data) {
		...
		render(json, $("#city_banner_render"),$("li.city_banner div.swiper-wrapper"));
	...
	init.prototype.conRecommend=function (data) {  
		...
		render(firstData, $("#l-r-tpl"), $("li#con_recommend_content_list_first_item"));
		render(json1.slice(1), $("#con_recommend_content_list_render"), $("ol.con_recommend_content_list"));
	}
	...
	init.prototype.bookRackList=function () {  
		$.ajax({
			...
			dataType: "json",
			success: function (data) {
				render(data, $("#book_rack_content_list_render"), $("ul.book_rack_con_list_render"));
